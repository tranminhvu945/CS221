...
# Lớp này dùng để đóng gói lại bộ tiền xử lý thành một transformer có thể sử dụng trong scikit-learn Pipeline.
...
if __name__ == '__main__':
    ...
    for i in range(1, 4):
        df = pd.read_csv(f"D:/CS221/Data/Original/ViCTSD_{dataset[i-1]}.csv")
        df_clean = preprocessor.preprocess_dataframe(df, text_col='Comment', out_col='Comment_clean')
        # save to csv
        df_clean.to_csv(f"D:/CS221/Data/Preprocessed/ViCTSD_{dataset[i-1]}-clean.csv", index=False)
